...
def test_epel_gpg_key(host):
    if host.system_info.release.startswith("7"):
        assert not host.package("gpg-pubkey-352c64e5-52ae6884").is_installed
    if host.system_info.release.startswith("6"):
        assert not host.package("gpg-pubkey-0608b895-4bd22942").is_installed


def test_epel_repo_enabled(host):
    cmd = "yum -v repolist epel"
    assert "repolist: 0" in host.check_output(cmd)
